...
function sendEmailUpdated($fname, $lname, $email)
{
    $fullname = $fname . " " . $lname;
    $mail = new PHPMailer(true);
    ...
    try {
        ...
        $email_body = "
            <img src='cid:logo' alt='Animal Haven Veterinary Clinic Logo' style='display: block; margin: 0 auto; width:20%; height:auto'>
            <h2>Dear $fullname,</h2>
            <h3>Your account has been updated in <b>PawPointment Platform</b> of <i>Animal Haven Veterinary Clinic</i></h3>
            <h4>Click the button below to log into your account</h4>
            <br><br>
...
function functionAddStaff($conn) {
    ...
    if ($add_staff_stmt->execute()) {
        successMessage("Staff Added successfully", "../staff.php");
...
function functionAddWalkIn($conn){
    do {
        $referral = mt_rand(100000000, 999999999); // Adjust the range
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $add_walkin_stmt = $conn->prepare($sql);
    $add_walkin_stmt->bind_param("issssssssssss", $referral, $fname, $lname, $mname, $address, $contacts, $animalName, $gender, $age, $birthdate, $quantity, $breed, $species);

    if ($add_walkin_stmt->execute()) {
        echo '<script>setTimeout(function() { window.location.href = "../login-form/add-appointment-p2.php?referral_no=' . $referral . '"; }, 1000);</script>';
    } else {
...
function checkreferralExists($conn, $referral){
    ...
    $check_stmt = $conn->prepare($sql);
    $check_stmt->bind_param("i", $referral);
    $check_stmt->execute();
    $check_stmt->bind_result($count);
    $check_stmt->fetch();
    $check_stmt->close();
...
function functionEditStaff($conn){
    ...
    $staff_query = "UPDATE staff SET firstname=?, lastname=?, MI=?, gender=?, position=?, email=?, pass=? WHERE staff_id = ?";
    ...
    $staff_stmt->bind_param("sssssssi", $fname, $lname, $mname, $gender, $position, $email, $password, $staff_id);

    if ($staff_stmt->execute()) {
        successMessage("Staff Updated Successfully", "../staff.php");
...
function functionUpdateStatus($conn){
    ...
    $paymentStatus = $_POST["remarks"];
    $comments = $_POST["comments"];

    $remarks = $paymentStatus . " " . $comments;
    $status_query = "UPDATE `appointment` SET `appointment_status`=?,`start_event_date`=?,`end_event_date`=?, `remarks`=? WHERE appointment_id = ?";
    $status_stmt = $conn->prepare($status_query);
    if (!$status_stmt) {
        die("Prepare failed: " . $conn->error);
    }
    $status_stmt->bind_param("ssssi", $status, $start_date, $end_date, $remarks, $appointment_no);
